...
def train_model(dataloaders, device, model, criterion, optimizer, scheduler, num_epochs=25, num_layers=8,
...
if __name__ == '__main__':
    ...
    """save config"""
    with open(f'./config/{num_layers}/{task_name}_finetune_{model_name}_{pretrained_name}_{eeg_type_choice}.json',
              'w') as out_config:
    ...
    save_name = f'{task_name}_finetune_{model_name}_{pretrained_name}_{eeg_type_choice}_b_{batch_size}_epoch_{num_epochs}_lr_{lr}_dataset_setting_{dataset_setting}'
